...
원인 불명 오류에는 `systematic-debugging`, 독립적인 병렬 lane에는 `multi-session-dev`를 추가한다. 이때 승인된 write 범위·commit 허용·task branch·수용 기준·검증 명령을 입력 계약으로 넘기고, 반환된 통합 결과·blocked 항목·남은 worktree를 이 프로토콜의 완료 절차에서 처리한다. lane 실행은 계획을 확정한 이 세션이 아니라 저장된 plan·계약만 읽는 새 세션에서 시작한다. 역방향 참조는 없다.
