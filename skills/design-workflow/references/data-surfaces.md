...
새 차트 방향을 만들기 전에 채택 후보의 공식 실행 예제를 브라우저에서 직접 조작한다. 예제 URL·확인한 행동·보이는 결과와 이번 시안에 가져올 표현을 짧게 기록한다. 선과 영역의 질감, 축·숫자의 위계, tooltip·crosshair, 범례 선택과 데이터 전환 중 실제 과업에 필요한 것을 지정한다. 문서·소스 읽기나 설치 성공을 동작 관찰로 기록하지 않는다. 공식 실행 예제에 접근할 수 없으면 제약과 실제 확인한 공식 영상·문서·소스 등의 대체 근거를 구분해 밝히고, 브라우저에서 확인했다고 주장하지 않는다. 기록 위치와 write 권한은 현재 모드를 따른다(`audit`는 대화로 보고).

새 차트 표현은 같은 대표 데이터와 지표로 **동작 시안 세 개**를 함께 보여주고 AI 추천·사용자 선택을 기록한다. 축·범례·tooltip·필터·업데이트 동작과 모바일을 직접 비교한다. 예: 간결한 업무형 / KPI와 추세 중심 / 비교·탐색 중심. 의미에 맞는 chart type은 먼저 결정하고 세 안을 채우려고 부적절한 원형·선·막대를 섞지 않는다. 세 안의 차이는 같은 차트 영역을 비교해도 표현·정보 위계·탐색 동작에서 드러나야 한다. 배치·색상·설명 이름만 바꾼 안으로 채우지 않는다. 필요한 표현을 선택하며, 예제의 모든 효과를 복제하거나 차트 의미를 바꾸지 않는다.
...
- 프로젝트 전체 적용을 요청받으면 실제 차트 사용처를 조사해 공통 토큰·wrapper와 종류별 예외를 구분하고 승인 범위에서 이관한다. 샘플 시안의 승인을 실제 데이터·권한·loading/empty/error 계약 검증으로 간주하지 않는다. 설치나 공통 wrapper 수정만으로 모든 화면 적용이 끝났다고 보고하지 않는다.
