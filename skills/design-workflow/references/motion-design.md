...
새 모션 방향은 채택할 공식 실행 예제를 브라우저에서 직접 조작한 뒤, 대표 행동과 채택할 상태 연결·움직임·완료 피드백을 지정한다. 예제 URL·직접 확인한 행동·보이는 결과를 현재 모드가 허용하는 기존 기록에 남긴다. 접근할 수 없으면 공식 영상·문서·소스 등 실제 확인한 대체 근거와 한계를 밝힌다. 문서 읽기나 설치를 직접 동작 관찰로 기록하지 않는다. 과업의 대기시간을 줄이는 판단과 모션 표현을 약화하는 판단을 구분한다. ERP라는 이유만으로 표현을 최소화하지 않고, 사용자가 요청한 인상과 반복 입력의 실측 결과로 강도를 정한다. 승인된 패턴의 작은 수정은 기존 근거를 재사용한다.
...
3. 새로운 모션 방향은 **같은 콘텐츠와 대표 행동으로 동작 시안 세 개**를 함께 보여준다. 기본 축은 빠른 반응·부드러운 연결·브랜드 표현이며 프로젝트에 맞게 구성한다. 각 안의 상태 연결·공유 요소 이동·강조와 완료 피드백 중 과업에 맞는 차이를 실제 동작으로 보여준다. 이름·배치·duration만 바꿔 세 안을 채우거나, 정지 이미지·외부 예제 링크만 제시하지 않는다.
...
- OS reduced motion을 초기 로드와 실행 중 설정 변경 모두에서 Motion·커스텀 JS·CSS·3D·영상에 반영한다. 앱에 수동 설정이 있으면 OS 설정을 켰다 꺼도 사용자 선택을 잃지 않게 한다. 공간 이동을 줄여도 정확한 최종값·선택·완료 피드백은 유지한다. 모바일은 hover 없이 중요한 기능을 조작할 수 있어야 한다.
