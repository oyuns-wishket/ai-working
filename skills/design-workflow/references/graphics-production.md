...
게이트·도구 기본값·규칙 소유는 [SKILL.md](../SKILL.md)를 따른다. 이 문서는 그래픽의 제작/전달 경로 선택과 경로별 검증 상세를 소유한다. UI 모션 도구 선택·timing은 [motion-design.md](motion-design.md), 3D runtime·scroll 소유권·자산 최적화는 [motion-and-3d.md](motion-and-3d.md)를 따른다.

`audit`이면 아래 제작·시안·웹 연결 절을 기존 자료·실행 중 화면의 읽기 전용 관찰에만 적용한다. 새 자산 생성·export·설치·source/기록 파일 쓰기 없이 관찰과 미실행 범위를 대화로 보고한다.
...
| 웹 전달 경로 | 이 경로를 고르는 결정 질문 | 적합한 요구 | 제작 → 웹 연결 | 먼저 확인할 차이 |
|---|---|---|---|---|
| 사전 렌더 3D/그래픽 영상 | 정해진 시점과 재생만으로 핵심 내용을 전달할 수 있는가? 그렇다면 영상 샘플로 확인한다. | 정해진 시점의 빛·재질·복잡한 효과를 보여줌 | Blender 등에서 렌더 또는 허용된 영상 제작 → encode → HTML video와 설명 | 새 시점/부품 조작 불가; 모바일 crop·재생 정책·전송/decode 비용 |
| 스크롤 영상 / 이미지 시퀀스 | 사용자가 준비된 장면의 앞뒤 진행만 제어하면 되는가? 그렇다면 seek/프레임 반응을 비교한다. | 앞뒤 scroll에 따라 준비된 장면 진행 | 카메라/장면 timeline → 영상 seek 또는 프레임 선택 → HTML 단계·CTA | seek 반응, keyframe/encode, decode·프레임 cache·메모리; 자유 회전과 구분 |
| 2D/2.5D 실시간 그래픽 | 뒷면·자유 3D 시점 없이도 입력 반응을 완성할 수 있는가? 그렇다면 2D/2.5D로 검증한다. | 글자·사진·캐릭터·입자·파동이 입력에 반응 | SVG/CSS/Canvas, Rive/Hana/Lottie, PixiJS 또는 shader → UI 입력/상태 | 원본의 뒷면·자유 3D 시점이 자동 생성되지 않음; runtime·효과 호환성 |
| 실시간 3D | 자유 시점·단면·부품·재질을 직접 바꾸는 것이 필수인가? 그렇다면 필요한 모델 구조와 runtime을 검증한다. | 자유 회전·단면·분해·재질/옵션·시점 변경 | 검수한 모델/scene → Three.js 등 renderer → 입력·scroll 연결 | 형상/부품 데이터, 기기 GPU·메모리·조명 차이 |
| 게임형 체험 | 입력과 규칙에 따라 결과가 달라지고 reset/상태 관리가 필요한가? 그렇다면 게임형 과업을 정의한다. | 이동·조립·수집·규칙에 따라 결과 변화 | 2D/3D 자산 + 입력·상태·규칙, 필요한 경우 물리 → reset/저장/UI | 게임은 상호작용 범위이며 위 renderer 위에 구현; 모든 장식에 게임 엔진 불필요 |
| 혼합형 | 구간별 필수 행동이 달라 한 전달 경로로 충족할 수 없는가? 그렇다면 아래 채택 조건을 모두 확인한다. | 브랜드 인상과 탐색·구매·설명을 함께 제공 | hero 영상 + 조작형 제품/그래픽 구간 + 일반 HTML UI 등 | 구간별 소유권·로딩·자원 해제, 내용/CTA 연속성 |

혼합형은 다음 조건을 모두 충족하면 채택한다. 잠금 해제 전에는 분리된 preview 샘플로 이 조건을 확인하고 결과·수용 예산·구현 계획을 기록한다. 최종 앱의 같은 검사는 구현 후 갱신하며, preview 통과만으로 웹 통합 완료를 보고하지 않는다.

1. 구간별 핵심 행동과 경로가 표로 정해져 있고, 단일 경로로 충족하지 못하는 이유가 있다. 예: hero의 정해진 재질 연출은 영상, 옵션 구간의 자유 회전·색상 변경은 실시간 3D다.
2. 짧은 source→export→브라우저 샘플에서 각 경로의 표현 보존과 전환을 확인했다. 영상이 항상 가볍거나 실시간이 항상 품질이 높다고 가정하지 않는다.
3. 구간별 progress·장면 시간·camera·속성의 소유자, 로딩 우선순위, 이탈 시 정지/해제가 정해져 있다. 같은 속성에 두 엔진을 연결하지 않는다.
4. 대표 모바일에서 동시 실행 비용을 실측하고 HTML 내용·CTA·focus·anchor의 연속성과 한 장면 실패 시 나머지 과업 완료를 확인했다.
5. 선택된 동작 시안에 혼합의 이점과 모바일/reduced motion 대안이 드러난다. 핵심 조작·호스팅·비용이 달라지면 달라지는 결정만 받는다.

조건이 충족되지 않으면 핵심 행동을 보존하는 단일 경로부터 검증하거나 미해결 조건을 남긴다. 3D 작업이 필요 없는 장면이면 모델·renderer를 먼저 설치하지 않는다. Motion/Anime.js는 [motion-design.md](motion-design.md)의 역할을 유지하고 엔진 내부 loop에 외부 timeline을 중복 연결하지 않는다.
...
3. **동작 시안:** 새 차트·모션·그래픽·3D 방향을 제작할 때는 해당 소유 참조의 **공식 실행 예제 직접 관찰 → 채택할 표현 지정 → 같은 데이터/콘텐츠로 동작 시안 정확히 세 개 → 결과 대조**를 따른다. 정지 이미지·외부 링크만으로 대체하지 않는다. 모션·차트·그래픽·3D가 들어가는 동적 시안은 **실제 동작**(재생/초기화, 클릭·필터·스크롤 조작)을 반드시 포함한다. [SKILL.md](../SKILL.md)의 기존 방향·composition 선택 안에 포함한다. 실제 시간 흐름과 요청한 클릭/scroll/입력을 체험하게 하고 replay/reset을 제공한다. 정지 mockup·외부 링크·stock placeholder만으로 최종 그래픽 품질이나 조작을 승인받았다고 기록하지 않는다. scoped 작업은 해당 구간만 비교하며 모델/영상/모션마다 별도 시안 세 묶음을 만들지 않는다.
   세 안은 장면 구조(카메라 경로·구간 순서)·조작/반응 방식·전달 경로 중 최소 두 가지에서 달라야 하며 색·조명 강도·easing만 바꾼 안은 부적합이다. 같은 콘텐츠·대표 행동과 승인된 범위 안에서 비교하며, 세 안의 각 쌍(A/B·A/C·B/C)에 이 기준을 적용한다.
...
[verification.md](verification.md)는 공통 실행 순서·증거 형식과 목적별 검증 요약을 소유한다. 이 절은 전달 경로별 검증 상세를 소유하며 scroll/3D runtime 상세는 [motion-and-3d.md](motion-and-3d.md)를 따른다. 모든 경로에서 대표 모바일·touch/keyboard, reduced motion, 느린/실패한 로드, route 재진입과 자원 해제를 확인한다. 정해진 FPS/용량 수치나 desktop 결과만으로 모바일 품질을 보장하지 않는다.
...
1. 공통 검증을 실행하고 실제 사용할 경로의 위 항목을 전부 확인한다. `small-feature`이면 대상 구간을 좁히되 desktop/mobile·상태·입력·실패·자원 해제 매트릭스를 줄이지 않는다.
2. 공식 예제에서는 채택한 표현의 시작/전환/완료와 대표 행동을 기준으로 대응 관계·조정 이유를 기록한다. 승인 동작 시안과 실제 웹은 같은 대상 route·viewport·data/state·theme·auth·장면·재생 위치·입력으로 대조한다(시안 URL은 달라도 대상 route·조건을 대응시킨다). 재생/초기화까지 확인한다. 정지 이미지는 구도 증거로만 쓰고 시간·입력 검증은 직접 조작/재생 기록으로 남긴다.
3. 성능 영향 여부는 [verification.md의 모드별 검증 매트릭스](verification.md#모드별-검증-매트릭스)로 판정한다. 영향이 있으면 기기·브라우저·network/cache·자산/프레임 조건과 측정 도구·구간을 고정하고 frame/seek/decode·입력 지연·메모리 중 해당하는 항목을 측정한다. 측정 불가 항목은 한계로 적는다.
4. reduced motion은 초기 로드와 실행 중 변경을 모두 확인한다. 공간 이동·자동 재생을 줄여도 설명·선택·완료 상태는 유지한다. 화면 이탈 후 영상·render loop·listener·cache가 남으면 소유자를 확인해 정리하고 재진입을 다시 검사한다.
5. 약속한 표현·행동이 빠지면 범위 내 코드/자산을 수정하고 영향받는 검사를 다시 실행한다. 미실행 검사와 미해결 결함이 있으면 검증 완료로 표시하지 않는다. 사용자 인상 판단은 자동 검사와 구분한다.

아래 상세 표는 [verification.md](verification.md)의 공통 검증 표 `결과`에 증거로 연결한다. 공통 표의 명령/route·조건·통과/실패/미실행/N/A·미실행 사유를 대체하지 않는다.

| 전달 경로·비교 조건 | 승인 시안의 채택 표현·행동 | 실제 웹 관찰·재생/조작 증거 | 성능·자원 해제 결과 | 판정·차이·후속 |
|---|---|---|---|---|
| route·viewport·data/state·입력/재생 위치 | 시작/전환/완료·모바일 대안 | 실제 URL·영상/기록·반영/조정/누락 | 조건·도구·전후 수치 또는 미측정 | 통과 / 실패 / 미실행·이유·재검증 |
